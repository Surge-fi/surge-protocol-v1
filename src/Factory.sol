...
contract Factory is IFactory {
    ...
    constructor(address _operator, string memory _poolSymbolPrefix) {
        ...
        require (bytes(_poolSymbolPrefix).length < 32);
        bytes32 packed;
        assembly {
            packed := mload (add (_poolSymbolPrefix, 31))
        }
        POOL_SYMBOL_PREFIX = packed;
    }
    ...
    function setPendingOperator(address _pendingOperator) external {
        require(msg.sender == operator, "Factory: not operator");
        require(_pendingOperator != address(0), "Factory: pendingOperator cannot be zero");
